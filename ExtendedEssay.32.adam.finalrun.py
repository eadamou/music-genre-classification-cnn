...
for i in range(3):
    ...
    for epoch in range(num_epochs):
        ...
        if np.argmax(valid_accuracies) == i*num_epochs+epoch:
            print('Saving the best model at %d epochs!' % epoch)
            torch.save(cnn.state_dict(), 'NEW_RESULTS/%d_best_model.adam.final.ckpt' %curr_batchsize)
            torch.save(hyperparams, 'NEW_RESULTS/%d_best_model_hyperparams.adam.final.pt' %curr_batchsize)

torch.save(Accuracies_lr,'NEW_RESULTS/%d_Accuracies_results.adam.final.pt' %curr_batchsize)
torch.save(Loss_lr,'NEW_RESULTS/%d_Losses_results.adam.final.pt' %curr_batchsize)
torch.save(LearningRate,'NEW_RESULTS/%d_Learningrates.adam.final.pt' %curr_batchsize)
torch.save(test_loader, 'NEW_RESULTS/%d_dataloader.adam.final.pth' %curr_batchsize)

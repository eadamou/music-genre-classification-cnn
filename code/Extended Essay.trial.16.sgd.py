...
for i in range(len(LearningRate)):
    ...
    for epoch in range(num_epochs):
        ...
        if np.argmin(valid_losses) == i*num_epochs+epoch:
            ...
            torch.save(cnn.state_dict(), '%d_best_model_sgd.ckpt' %curr_batchsize)
            torch.save(hyperparams, '%d_best_model_hyperparams_sgd.pt' %curr_batchsize)

torch.save(Accuracies_lr,'%d_Accuracies_results_sgd.pt' %curr_batchsize)
torch.save(Loss_lr,'%d_Losses_results_sgd.pt' %curr_batchsize)
